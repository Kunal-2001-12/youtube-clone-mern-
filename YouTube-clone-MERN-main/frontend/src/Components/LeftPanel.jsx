    ...
    <>
      ...
      >
        ...
        <div
          ...
          style={isSwitch === false ? { display: "block" } : { display: "none" }}
        >
          <Signup />
      ...
      {togglepanel && (
        <div
          ...
          style={{ opacity: closePanel ? 0 : 1, pointerEvents: closePanel ? "none" : "auto" }}
        />
      )}
      ...
      <div
        ...
        role="dialog"
      >
        <div className={theme ? "panel-topdata" : "panel-topdata light-mode"} style={{ display: 'flex', alignItems: 'center', padding: '12px 16px 8px 8px', borderBottom: '1px solid #222', minHeight: 56 }}>
          <img
            src={theme ? Logo : Logo2}
            alt="YouTube"
            style={{ height: 28, width: 'auto', marginRight: 12, cursor: 'pointer', objectFit: 'contain', filter: theme ? 'none' : 'grayscale(1)' }}
            onClick={() => { window.location.href = "/"; }}
          />
          <MenuRoundedIcon
            fontSize="large"
            style={{ color: theme ? "white" : "black", marginLeft: 'auto', cursor: 'pointer' }}
            ...
            tabIndex={0}
            onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') { setClosePanel(true); setTimeout(() => { document.body.classList.remove('bg-css'); }, 250); setTimeout(() => { setClosePanel(false); setTogglePannel(false); }, 350); } }}
          />
        </div>
        <div
          className={
