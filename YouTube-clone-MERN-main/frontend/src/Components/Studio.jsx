...
function Studio() {
  ...
  useEffect(() => {
    // Check if user has a channel after login/signup
    if (user?.email) {
      fetch(`${backendURL}/getchannel/${user.email}`)
        .then((res) => res.json())
        .then((data) => {
          if (data.hasChannel === false) {
            // Prompt user to create channel if not exists
            setisChannel(false);
            setIsClicked(true); // Open channel creation modal
          }
        })
        .catch(() => {
          setisChannel(false);
  ...
  // --- ENFORCE CHANNEL CREATION BEFORE UPLOAD ---
  // Block all upload triggers if user has no channel
  // Only allow upload modal to open if isChannel === true

  // Refined upload modal open logic
  const handleOpenUploadModal = () => {
    if (isChannel === true) {
      setIsClicked(true);
    } else {
      toast.warning("You must create a channel before uploading videos.", {
        position: "top-center",
        autoClose: 1500,
        theme: theme ? "dark" : "light",
      });
  ...
  // In the return JSX, update upload triggers:
  // <div ... onClick={handleOpenUploadModal} ...>
