...
function Branding() {
  ...
  const handleDeleteChannel = async () => {
    if (!window.confirm("Are you sure you want to delete your channel? This action cannot be undone and all your videos will be lost.")) return;
    setDeleteLoading(true);
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(`${backendURL}/deletechannel/${user?.email}`, {
        method: "POST",
        headers: { "Content-Type": "application/json", ...(token ? { Authorization: `Bearer ${token}` } : {}) },
      });
      const data = await res.json();
      if (data.success) {
        alert("Channel deleted successfully. You will be logged out.");
        navigate("/");
    ...
    <>
      ...
      >
        ...
        <div style={{ marginTop: 40, textAlign: "center" }}>
          ...
          <p style={{ color: '#fe3e2b', marginTop: 8, fontSize: 14 }}>
            Warning: This will permanently delete your channel and all its content.
          </p>
        </div>
      </div>
    </>
