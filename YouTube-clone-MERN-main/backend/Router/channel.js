...
Channel.post("/savechannel", requireAuth, async (req, res) => {
  try {
    const {
      email,
      ChannelName,
...
Channel.post("/deletechannel/:email", async (req, res) => {
  try {
    const email = req.params.email;
    // Remove user and all their videos
    const user = await userData.findOneAndDelete({ email });
    await videodata.deleteMany({ email });
    await TrendingData.deleteMany({ email });
    if (!user) {
