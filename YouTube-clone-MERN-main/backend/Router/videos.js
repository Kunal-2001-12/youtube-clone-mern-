...
const isValidObjectId = require("../lib/isValidObjectId");
const Videos = express.Router();

Videos.post("/publish", requireAuth, async (req, res) => {
  try {
    const {
      videoTitle,
      videoDescription,
    ...
    let videos = await videodata.findOne({ email: user.email });

    // --- ENFORCE: User must have a channel to upload ---
    if (!user || user.hasChannel !== true) {
      return res.status(403).json({ message: "You must create a channel before uploading videos." });
    }

    if (user) {
      // Ensure user.videos and user.thumbnails are arrays before pushing
      if (!Array.isArray(user.videos)) user.videos = [];
      if (!Array.isArray(user.thumbnails)) user.thumbnails = [];
...
Videos.get("/getvideos", async (req, res) => {
  try {
    ...
    const videoID = videos.flatMap((video) =>
      video.VideoData.map((data) => data._id)
    );
...
Videos.get("/getuserimage/:email", async (req, res) => {
  try {
    ...
    const channelIMG = user.profilePic || null;
    res.json({ channelIMG });
  } catch (error) {
    console.error(error);
...
Videos.get("/videodata/:id", async (req, res) => {
  try {
    ...
    const videoDoc = await videodata.findOne({ "VideoData._id": id });
    if (!videoDoc || !Array.isArray(videoDoc.VideoData)) {
      return res.status(404).json({ error: "Video not found" });
    }
    const video = videoDoc.VideoData.find((v) => v._id && v._id.toString() === id);
    if (!video) {
